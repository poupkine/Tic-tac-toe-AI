...
def minimax_advice(minimax_board, depth, is_maximizing):
    if check_win(1, minimax_board):
        return float('inf')
    elif check_win(2, minimax_board):
        return float('-inf')
    elif is_board_full(minimax_board):
        return 0
    if is_maximizing:
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLUMNS):
                if minimax_board[row][col] == 0:
                    minimax_board[row][col] = 1
                    score = minimax_advice(minimax_board, depth + 1, is_maximizing = False)
                    minimax_board[row][col] = 0
                    best_score = max(score, best_score)
        return best_score
    else:
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLUMNS):
                if minimax_board[row][col] == 0:
                    minimax_board[row][col] = 2
                    score = minimax_advice(minimax_board, depth + 1, is_maximizing = True)
                    minimax_board[row][col] = 0
                    best_score = min(score, best_score)    
    return best_score


def best_move_advice():
    best_score = -1000
    move = (-1, -1)
    for row in range (BOARD_ROWS):
        for col in range(BOARD_COLUMNS):
            if board[row][col] == 0:
                board[row][col] = 1
                score = minimax_advice(board, 0, False)
                board[row][col] = 0
                if score > best_score:
                    best_score = score
                    move = (row, col)
    if move != (-1, -1):
        print(('Best row: ', move[0] + 1, "Best column: ", move[1] + 1 ))
        return None
    return None
...
while True:
    for event in pygame.event.get():
        if count >1 and player == 1 and step > 1:
            if player == 1:
                
                best_move_advice()
                
        else:
            print("Best first step is row 2, col 2")
            init = False
        count += 1
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            #print("row:", mouseX, "column:", mouseY)
